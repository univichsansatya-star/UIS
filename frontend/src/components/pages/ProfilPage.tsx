...
import type { CampusProfileSection, VideoTour, VisionMission as VisionMissionType } from '../../types';
...
export const ProfilPage: React.FC<ProfilPageProps> = ({ onNavigate }) => {
  ...
  const [videoTour, setVideoTour] = useState<VideoTour | null>(mockVideoTour);
  const [visionMission, setVisionMission] = useState<VisionMissionType | null>(mockVisionMission);
  const [campusProfiles, setCampusProfiles] = useState<CampusProfileSection[]>(mockCampusProfiles);

  useEffect(() => {
    getRectorGreeting().then(setRectorGreeting);
    getCampusStats().then(setCampusStats);
    getVideoTour().then((data) => setVideoTour(data ?? null));
    getVisionMission().then((data) => setVisionMission(data ?? null));
    getCampusProfiles().then((data) => setCampusProfiles(Array.isArray(data) ? data : []));
  }, []);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-8 py-8 space-y-16">
      ...
      {videoTour && (
        <section className="bg-white rounded-3xl p-8 sm:p-12 border border-gray-200 shadow-sm space-y-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <span className="text-xs font-mono-code text-[#00ADF1] uppercase font-semibold">Video Tour</span>
              <h2 className="font-display font-bold text-2xl text-[#17356B]">{videoTour.title}</h2>
            </div>
          </div>
          <iframe src={videoTour.youtubeEmbedUrl} className="w-full aspect-video rounded-2xl" allowFullScreen />
          {videoTour.description && <p className="text-gray-600 text-sm leading-relaxed whitespace-pre-line">{videoTour.description}</p>}
        </section>
      )}
      ...
      {visionMission && (
        <section className="bg-white rounded-3xl p-8 sm:p-12 border border-gray-200 shadow-sm space-y-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <span className="text-xs font-mono-code text-[#00ADF1] uppercase font-semibold">Visi & Misi</span>
              <h2 className="font-display font-bold text-2xl text-[#17356B]">Profil Institusi</h2>
            </div>
          </div>
          <div className="space-y-4">
            <p className="text-gray-700 text-sm sm:text-base leading-relaxed">{visionMission.vision}</p>
            {visionMission.missions.length > 0 && (
              <ul className="space-y-3 text-sm text-gray-700">
                {visionMission.missions.map((mission, index) => (
                  <li key={`${mission}-${index}`} className="flex items-start gap-2.5">
      ...
      {campusProfiles.length > 0 && (
        <section className="space-y-6">
          <div className="space-y-2">
            <span className="text-xs font-mono-code text-[#00ADF1] uppercase font-semibold">Campus Profile</span>
            <h2 className="font-display font-bold text-2xl sm:text-3xl text-[#17356B]">Profil Kampus</h2>
          </div>
          <div className="space-y-6">
            {campusProfiles.map((profile) => (
              <div key={profile.id} className="bg-white rounded-3xl p-8 sm:p-12 border border-gray-200 shadow-sm">
                <h3 className="font-display font-bold text-2xl text-[#17356B] mb-4">{profile.title}</h3>
                <div className="prose prose-slate max-w-none text-gray-700 text-sm sm:text-base leading-relaxed" dangerouslySetInnerHTML={{ __html: profile.content }} />
              </div>
            ))}
          </div>
        </section>
